...
describe('Template ↔ source-name consistency', () => {
  ...
  const NON_TEMPLATE_NAMES = new Set([
    ...
    // (#1778): 'llama' is a real `.container`-Quadlet service (no
...
describe('ServiceManager.STACK_MIGRATIONS map shape', () => {
  ...
  it('keys reference real templates, predecessors are no-longer-existing names', () => {
    // STACK_MIGRATIONS lives in lib/services/lifecycle/migrations.ts since
    // the #2741 split — serviceLifecycle.ts is now a facade that re-exports it.
    const sm = fs.readFileSync(path.join(BACKEND_SRC, 'lib', 'services', 'lifecycle', 'migrations.ts'), 'utf-8');
    const block = sm.match(/STACK_MIGRATIONS:\s*Record<string,\s*string\[\]>\s*=\s*\{([\s\S]*?)\n\s*\};/);
    expect(block, 'STACK_MIGRATIONS block not found in lib/services/lifecycle/migrations.ts').toBeTruthy();
