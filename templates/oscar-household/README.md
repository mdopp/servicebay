...
1. **Schema init.** Runs the migration sidecar (`ghcr.io/mdopp/oscar-household-init`, built from [`src/schema/`](src/schema/)) against `/var/lib/oscar/oscar.db` on every pod start. Creates `system_settings`, `cloud_audit`, `voice_embeddings`. Idempotent.
2. **Skill mount.** The bind-mounted `/var/lib/oscar` volume holds OSCAR's [`src/skills/`](src/skills/) clone (placed there by ServiceBay's registry sync). The ServiceBay `hermes` template mounts the same path into the Hermes container at `/opt/data/skills/oscar`, so Hermes picks the skills up alongside its built-in Skills Hub.
