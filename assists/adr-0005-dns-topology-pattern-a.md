---
title: "ADR 0005 — DNS topology: the router hands out AdGuard as the LAN DNS (Pattern A); deterministic resolution beats public fallback"
whenToUse: "A hostname on this box does not resolve the way you expect: a service subdomain does not reach the box, an SSO/OIDC callback or an LDAP fetch fails on the name, or a DNS probe warns that the router hands out the wrong resolver — and you need to know which topology is the intended one."
kind: adr
tags: [adr, decision, dns, adguard, network]
---
...
- **Related:** [ADR 0001](adr-0001-authentication-via-authelia-sso-or-lldap.md), [ADR 0006](adr-0006-authelia-apex-deny-vs-wildcard.md)
