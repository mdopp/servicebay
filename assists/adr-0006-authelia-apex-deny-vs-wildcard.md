---
title: "ADR 0006 — Authelia: the bare apex is default-deny; only `*.<domain>` subdomains are `one_factor`"
whenToUse: "You are writing something that asks Authelia who a visitor is — a forward-auth call, an SSO probe, a verify check — and it returns no identity or a 400; or you are deciding which hostnames get an access-control rule and why the bare apex domain is denied while subdomains are not."
kind: adr
tags: [adr, decision, authelia, sso, forward-auth, access-control]
---
...
- **Related:** [ADR 0001](adr-0001-authentication-via-authelia-sso-or-lldap.md), [ADR 0005](adr-0005-dns-topology-pattern-a.md)
