---
title: "ADR 0004 — Installing/redeploying a service never wipes other services; system-wide reset is factory-reset-only"
whenToUse: "You are writing or reviewing anything that deletes, wipes, resets or restores — a clean-install toggle, a reinstall path, a wipe-mode flag, an auto-restore — and need to know how far one service's install is allowed to reach and what may only ever happen behind a factory reset."
kind: adr
tags: [adr, decision, install, destructive, reset, restore]
---
...
- **Related:** [ADR 0002](adr-0002-tiered-backup-nas-config-vs-bulk-drive.md), CREDENTIAL_SELF_HEAL.md
...
- Core/atomic-wipe stacks (`basic` = NPM + LLDAP/Authelia + AdGuard) can only be
  wiped via Factory Reset; the TUI/UI blocks unchecking them ([ADR 0008](adr-0008-tui-desired-state-and-journey.md)).
