...
*   **Tool surface** (`packages/backend/src/lib/mcp/tools/`, one module per tool group, 63 tools):
    ...
    *   *Mutate*: `manage_service` (`action: start\|stop\|restart\|force-update` — force-update re-checks the registry, re-pulls and recreates the containers, #2397), `deploy_service`/`update_service_yaml`/`delete_service`/`rename_service`, `create_proxy_route`/`remove_proxy_route`, `run_backup`/`restore_backup`/`list_backups`, `create_health_check`/`delete_health_check`/`run_check_now`, `update_config`, `exec_command`, `refresh_agent`.
...
*   **Secret hygiene**: `get_config` scrubs *structurally*, not by path (#2404) — any field whose key name looks secret-shaped (`password`, `secret`, `token`, `key`, `hash`, `credential`, …) is redacted at any depth, so `gateway.password`, `lldap`/`adguard`/`reverseProxy.npm` passwords and every future per-service credential are covered; secret-bearing collections (`installedSecrets`, `installManifest.credentials`) are replaced wholesale with an entry count. Inputs that flow into a shell command (container id, service name, domain) are constrained with regex schemas to prevent command injection through the `exec_command` / `get_logs` paths.
