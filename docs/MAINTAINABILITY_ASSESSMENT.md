...
| MSW mock handlers vs routes | was 26 / 174, used by **no test** — the whole layer is gone (#2729) | n/a |
...
|---|---:|---|
| Nine HTTP routes with no caller: `system/certs/archive` (120), `system/token-requests` + `[id]` (80), `system/learning-proposals/[id]` + `[id]/approve` (96), `agents/restart` (38), `system/last-crash` (28), `system/keys/bcrypt` (27), `system/install/persist-secret` (24) | 413 | Zero hits for each path in frontend (non-route), backend, api-client, both workers, `docs/`, `assists/`, `scripts/`, `install-sb.sh`, `.github/`. The MCP tools for token requests and learning proposals call `lib/` directly; there is no review UI for proposals. The companion app uses `/napi`, not these. **Answered + done (#2728, 2026-09-02):** no external client hits them; all nine route files are deleted and the stale `keys/bcrypt` name-drop in `templates/adguard/post-deploy.py` is corrected — the bcrypt hash is generated by `lib/install/manifestAssembler.ts`, never over HTTP. |
| `app/(dashboard)/dev/components/page.tsx` — the component catalog (#2354) | 325 | Deliberately hidden dev tool; no nav entry, no link, ships in the production bundle behind a `notFound()`. **Answered + done (#2729, 2026-09-02):** removed, with its test and its invariant rule. The `@/components/ui` barrel is the discovery surface. |
| MSW dev mocks: `src/mocks/*`, `providers/MockProvider.tsx` (mounted unconditionally in `app/layout.tsx`), the `msw` dependency | 358 | `NEXT_PUBLIC_USE_MOCKS=1` is set in exactly one place: the `dev:frontend` script. No test imports the handlers. 26 of 174 routes mocked, so the "frontend without a box" workflow is already 85 % broken. **Answered + done (#2729, 2026-09-02):** removed. `npm run dev:frontend` still starts — it now talks to a real backend. |
| `scripts/enable-nvidia.sh` | 107 | Referenced from nowhere in the shipping tree (package.json, CI, docs, other scripts) — only from one assertion in `tests/backend/install_nvidia_cdi_timer.test.ts` and one hint string in the Butane asset. **Answered + done (#2729, 2026-09-02):** deleted; the GPU-retrofit know-how is the assist `recipe-retrofit-nvidia-gpu-on-a-running-box`, which is delivered at runtime and can be corrected without a release. |
...
| Pending-approvals list | **Done (#2735).** `McpSection` mounts `usePendingApprovals()` + `PendingApprovalList` from `components/PendingApprovalsCard.tsx`; it owns no poll, no rows and no grace period of its own. | `McpSection` uses the hook and the shared list. | ~120 | Low |
