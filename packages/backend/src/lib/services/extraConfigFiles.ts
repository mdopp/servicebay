...
// #3044 — the sudo-write ownership repair moved to its own module so the
// reverse-proxy writer could use it too. Re-exported here because the tests
// that pin its pure halves were written against this file.
import {
    ensureDirAndProbeOwner,
    realignSudoWrite,
    extractStdout,
    type FileWritingAgent,
} from './sudoWriteOwnership';

export { parseOwnerReference, ownershipRepairTargets } from './sudoWriteOwnership';
