// Immich admin API-key reconcile (#1904) — mint-once + adopt, mirroring the
// Hermes reconcile (#1761).
//
// The disk-import External-Library provisioning (immichLibraries.ts, Decision A)
// needs ONE stored Immich ADMIN API key — never per-user keys, never asking
// users. Immich keys can't be pre-generated externally (only Immich mints them),
// so we adopt-or-mint at the box: log in as the seeded admin (the same
// credentials the immich post-deploy seeds), reuse an existing ServiceBay-
// managed key if one is already on the account, otherwise mint a fresh one, and
// persist it under `installedSecrets.IMMICH_ADMIN_API_KEY` (encrypted at rest)
// via `persistSingleSecret` — exactly like the Hermes key.
//
