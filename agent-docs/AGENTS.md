...
| `servicebay progress [<job-id>] [--node <name>]` | How an install is going — or, with nothing running, **how the last one ended**: phase, error and the last log lines. Takes the id `install` printed. Exit 11 when the run ended in error, crashed or aborted. |
...
| `servicebay source-add <repo-url> [--name <name>] [--branch <ref>]` | Register a repo as a **template source**, so its templates can be installed at all. **CHANGES the box**; needs `mutate`. Reports whether the source actually synced — **exit 12 means registered but the sync did not succeed**, i.e. nothing to install from it yet. |
