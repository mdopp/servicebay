...
- **Never touch `packages/**`, `tools/**`, `scripts/**`, `.claude/skills/**`, `.claude/state/**` (the pipeline's broker cache and result files), or any file outside the fileset above.** Those are the builder's. Editing them risks a working-tree collision and a lost commit.
...
Track progress at `.claude/state/docs-coherence-state.json` (note: a **separate** file from the autoloop's broker cache `autoloop-cache.json`, so the two loops never write the same state). Shape:
