...
- **Recurrence** — the issue names a **predecessor** ("← #N", "same as #N", "regression of #N", "this was fixed in #N and is back") → `gh issue edit <N> --add-label recurrence`, and the unit's `acceptance` must demand **a test or an invariant for the CLASS, not the case**. 32.8 % of commits since 2026-06-01 were `fix(…)`, and the repeat pairs (#2717←#1298, #2435←#2415, #981←#930) are all the same shape: the first fix landed with no gate, so the form came back. A recurrence unit closes only when the diff carries the gate — a test asserting the *shape* (every route on this gate, every caller of this helper, the invariant in `scripts/check-invariants.ts` / `.dependency-cruiser.cjs`), never a single-case assertion on the one line that broke. Write that into `acceptance` explicitly ("a test that fails for ANY … , not just #N's path"); the builder builds the acceptance, so a vague one ships another ungated fix. If the class genuinely can't be tested, say so in `acceptance` and name what *is* asserted — don't leave it implicit.
...
Record each unit with `npm run autoloop:queue -- plan '{"id":…,"kind":…,"issues":[…],"theme":…,"region":…,"scope":…,"acceptance":…,"gate":…,"security":false,"notes":…}'` (`status`/`pr` default; `plan` also labels every member issue `autoloop:queued`). `kind` is validated — one of `issue`, `cluster`, `lint-sweep`, `dep-updates`, `consolidation`; anything else is refused (exit 2) rather than queued as a typo. `scope` = one line on what to do; `acceptance` = how the builder knows it's done. The builder's `next` serves planned units **by id**, so encode selection priority (Step 4) as a two-digit id prefix — `01-fe-layout`, `02-install-creds`.
...
- **(d) Consolidation slot — at most ONE per batch** (#2746). Once no user issue waits, the loop may take a single unit from the **measured** reports: `npm run check:lint-ratchet` (the ratchet's own numbers), `npm run check:frontend-dup`, `npm run check:invariants`. Pick the one finding with the largest measured number, and `plan` it as `{"id":"9x-consolidate-<slug>", "kind":"consolidation", "issues":[], "region":"<the files the report names>", "scope":"…", "acceptance":"the report's number drops from <a> to <b> and the ratchet is tightened to match", "gate":"normal"}`. Ratchet direction is one-way — **tighten `scripts/check-invariants.ts` / `.dependency-cruiser.cjs`, never loosen** — otherwise the slot launders a regression as consolidation.
  The rule is **enforced by the broker, not by this paragraph**: `plan` refuses a `consolidation` unit while any user-issue unit is still `planned` (`user issues are waiting …`) and refuses a second one in the same batch (`at most one per batch`), and `summary` prints `consolidation_slot` so a firing can see the slot is spent. A refusal (exit 2) is the answer, not something to work around — fall back to (a)/(b)/(c).

**Autonomous default order:** (b) if any `autoloop:blocked` issue is open; else (c) if `summary.last_codebase_eval` is older than ~5 firings; else (d) if the batch's `consolidation_slot` is still free; else (a). Record the chosen track with `note`.
...
## Never
- Never let a `recurrence` issue close on a fix without a **class-level** test/invariant — a second single-case fix is how the pair came back (#2746).
