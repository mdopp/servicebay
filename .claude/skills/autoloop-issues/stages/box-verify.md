...
1. **Pre-pull `:dev`, then flip.** While polling for the `:dev` image to land (Step 2), kick off a **background pull on the box** so the flip is a cache-hit rather than a 5–10 min pull on the restart critical path: `systemd-run --user --unit=sb-prepull-dev podman pull ghcr.io/mdopp/servicebay:dev` (survives the 30 s MCP / 5-min exec caps — memory `feedback_box_update_slow_pull_timeout`). Once the image is present, **flip to dev**: `sb channel dev` (or `POST /api/system/channel`). Invocation/payload: `tools/sb/internal/rest/channel.go`, `tools/sb/internal/ui/channel.go`, `packages/backend/src/lib/servicebayChannel.ts`.
2. **Wait (bounded) for the dev image to land, then for health.** Poll the box's running image/version until it matches the newest merged SHA. **Timeout ≤15 min** (release.yml build + box pull + restart; the Step-1 pre-pull shrinks the pull leg to a cache-hit). If it never lands, treat as a verify failure (step 5, reason "dev image didn't land"). Once it lands, **poll `GET /api/health` until ready (bounded) BEFORE firing any probe** — probes fired during the restart window just burn `--max-time` timeouts and prove nothing.
3. **Verify.** Run `/verify` against `<SERVICEBAY_BOX>`, exercising the merged path-mandated changes (`box_verify.detail` names which paths). Run **independent probes concurrently**, each bounded (`curl --max-time 15` / `timeout N`); never an unbounded read (memory `reference_mcp_destroy_tier_approval_flow`). Sweep stray `*.bak` before reinstall-style checks (memory `feedback_hermes_config_bak_selinux`).
