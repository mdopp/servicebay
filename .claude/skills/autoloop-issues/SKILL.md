...
4. **Channel check — is the box stranded on `:dev`? (#2826)** The FULL box-verify path flips the box to `:dev` and back; the harness's `finally` covers a dying agent, but **not its container dying** — the orchestrator and its background harness run *inside* `claude-dev`, so a verify whose probes upgrade/restart that service recreate the container, kill the harness's process tree, and leave the box on `:dev` (2026-09-05). The repair is one script call, not a hand-flip:
   ```bash
   npm run autoloop:dev-verify -- --recover
   ```
   It reads MCP `get_channel` plus the harness's in-flight marker (`.claude/state/dev-verify-inflight.json`, written before the flip on the persistent volume so it outlives the container) and flips back through `set_channel latest` **only** when the box is on `:dev` and no live harness owns the flip — a running verify is left alone. Exit 0 = not stranded (or repaired), 2 = the box did not answer `get_channel` (nothing flipped), 5 = the box is on `:dev` and the repair flip failed → hard alert. When it repairs, log it: `npm run autoloop:queue -- note "recovered stranded :dev flip (<reason from the result line>)"`.
5. **Read status.** `npm run autoloop:queue -- summary` (compact — batch, verify, planned count, gh label counts). On a cold start (no cache), `… -- rebuild --release-pr <n>` reconstructs it from GitHub. `… -- mirror --pr <n>` prunes the cache + re-projects the release-PR label.
6. **Fold in any background Box-Verify result.** If `.claude/state/box-verify.json` exists, the background agent finished: fold it in with `npm run autoloop:queue -- verify-set <sha> <status> --detail "<…>" --pr <release PR>` (you are the single writer of the verify field), then **delete the result file**. `verify-get` auto-resets a `verifying` entry stuck >20 min (the agent died → it relaunches).
7. **Release-PR gate.** `gh pr list --head release-please--branches--main--components--servicebay --state open --json number,title`. If a release PR is open:
